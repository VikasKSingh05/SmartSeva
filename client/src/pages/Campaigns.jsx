...
const Campaigns = () => {
  const { campaigns, isLoading, address } = useStateContext();
  ...
  // Original full list rendering commented out as requested
  // return (
  ...
  // Limited view previously used; now show My Campaigns and All Other Campaigns
  const lower = (v) => String(v || "").toLowerCase();
  const mine = (campaigns || [])
    .filter((c) => lower(c.owner) === lower(address))
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 min-h-[70vh]">
      ...
      <div className="space-y-10">
        <DisplayCampaigns title="My Campaigns" isLoading={isLoading} campaigns={mine} />
        <DisplayCampaigns title="All Other Campaigns" isLoading={isLoading} campaigns={others} />
      </div>
    </div>
  );
};
